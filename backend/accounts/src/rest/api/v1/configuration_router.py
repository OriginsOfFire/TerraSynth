...
from src.rest.schemas.configuration_schema import ConfigurationCreateSchema, ConfigurationSchema
...
@configuration_router.post('/configuration/', status_code=status.HTTP_201_CREATED, response_model=ConfigurationSchema)
async def create_configuration(
    ...
    return await ConfigurationManager.create(
        input_data=input_data,
        session=session
    )


@configuration_router.delete('/configuration/{config_id}', status_code=status.HTTP_200_OK)
